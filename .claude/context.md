...
```
ANTHROPIC_API_KEY=          — Claude API key (only key required)
```

All other data sources are key-less / free:
  /api/price       gold-api.com
  /api/signals     yahoo-finance2 npm
  /api/technicals  yahoo-finance2 npm
  /api/calendar    ForexFactory weekly JSON
...
- [FREE-NEWS] /api/news rewritten to use Google News RSS — replaces the keyed newsdata.io upstream with a free no-key XML feed (https://news.google.com/rss/search?q=...). Same impact + sentiment tagging, same NewsArticle output shape, same mock fallback on outage. NEWSDATA_API_KEY removed from .env.example. Verified: 10 fresh real headlines (Fortune, Investing News Network, Yahoo Finance, KITCO, CME Group), HIGH/MEDIUM impact correctly tagged from FOMC/Fed keywords, ~1s response time. Only ANTHROPIC_API_KEY remains as a required external key for /api/analyze + /api/briefing.
- [CHART-FIX] GoldChart preserves user zoom across 60s polls — framedTimeframeRef tracks the last auto-framed TF; the candle-data effect now only calls timeScale().fitContent() when active TF changes, not on every refresh. Setting setData() alone preserves the visible range, so the user's pan/zoom holds through the 60s technicals poll cycle.
- [SPRINT-12] Integration + consistency pass — centralised utilities, animation parity, tooltip coverage. lib/utils.ts gains parsePrice(str) — strips $/comma/space, regex-extracts the first number, returns 0 for "——" or unparseable; AnalysisPanel.parseFirstNumber, lib/history.ts.parseFirstNumber, and lib/hooks/useAlerts.ts.parseFirstNumber all now delegate to it (each kept as a thin local wrapper to preserve their per-call-site return contract). app/globals.css gains slideIn keyframe + .slide-in class; JournalPanel aside applies slide-in className for the right-edge enter animation; BriefingModal applies the existing fade-in className (centred modals don't suit a slide-x enter). components/TradingViewChart.tsx wraps the legend's "● N PATTERNS" chip and the "● ALIGNED" trend-alignment indicator in Tooltips with copy explaining what they mean (Tooltip imported from @/components/Tooltip). State/wiring audit confirmed: page.tsx already lifts goldPrice + chartLevels + lastAnalysisResult + isJournalOpen + isBriefingOpen + alertStackHeight; AnalysisPanel calls onLevelsUpdate + onAnalysisComplete after each successful run; useHistory dispatches + listens for 'historyUpdated' so MEMORY tab refreshes cross-component; useBriefing self-fetches all upstream APIs; alertStackHeight reserves layout space below PriceBar so the fixed banner doesn't overlay the SignalsPanel. /api/technicals returns 200 in ~350ms; /api/price + /api/briefing also smoke-tested green. tsc --noEmit clean.
- [SPRINT-11] Confidence calibration display — answers "when Claude says HIGH, how often is it right?" New lib/calibration.ts: computeCalibration() reads getHistory(), prefers outcome4H over outcome2H, classifies HIT_TARGET=correct / HIT_STOP=incorrect / OPEN+INCONCLUSIVE=excluded, calcAccuracy returns null below 3 outcomes/bucket (so we render "——" not misleading "0%"), isCalibrated flips at >= 10 total decided outcomes. AnalysisPanel inserts a CALIBRATION block between the catalyst and the action button — header with Tooltip ("How accurate the system's confidence levels have been... Requires 10+ completed trade outcomes.") + outcome count, !isCalibrated state shows "X/10 outcomes needed" + a 2px progress bar #444444 fill, isCalibrated state renders three rows (HIGH green/MEDIUM amber/LOW red badges, 4px bar with width = accuracy%, fill colored per ≥65 green / ≥50 amber / red, accuracy text "——" when null), plus a calibration insight line: red "⚠ System accuracy below 50% on HIGH confidence..." when high<50; amber "⚠ HIGH confidence underperforming MEDIUM..." when high<medium; green "HIGH confidence is performing as expected." when high>=medium; muted "More data needed per confidence level." when all rows are null. CalibrationRows extracted as a sub-component. useEffect re-runs computeCalibration on data?.generatedAt + history.history changes. tsc --noEmit clean.
- [SPRINT-10] Session playbooks in Claude prompt — pure prompt engineering, only /api/analyze/route.ts changes. System prompt gains SESSION-SPECIFIC PLAYBOOKS block: TOKYO (low volume, false breakouts, reduce confidence one tier unless 7+ signals, 0.5-1x ATR move size), LONDON (high volume, LONDON OPEN FALSE BREAK RULE — entryType=WAIT before 07:30 UTC, after 07:30 EMA pullback ideal, 1-2x ATR), NY/LONDON OVERLAP (peak volume, full position when clearToTrade, weight trend signals heavier on continuation, 1.5-3x ATR), NEW YORK (declining volume, only 7+ confluence, 0.5-1.5x ATR), OFF-HOURS (FLAT unless 8/8 + major catalyst). Rules to put ATR-multiple range in holdTime ("1-3 hours ($28-55 expected range)") and explicit guidance for London false break: entryType=WAIT + entryTiming note. User message gains "Current UTC time / hour / minute" lines at the top so Claude doesn't have to re-derive time from the session string for the 07:30 rule. AnalysisResult JSON shape unchanged — holdTime is already free-form. tsc --noEmit clean.
